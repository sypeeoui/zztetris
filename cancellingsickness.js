...
function newPiece() {
	while (queue.length < 10) {
        ...
        let cancelStreak = document.getElementById("CancelStreak").value;
        var penaltyPieces = [];
        if (cancelStreak >= 20) penaltyPieces.push("O");
        if (cancelStreak >= 30) penaltyPieces.push(Math.random() < 0.5 ? "L" : "J");
        if (cancelStreak >= 40) penaltyPieces.push(Math.random() < 0.5 ? "S" : "Z");
        if (cancelStreak >= 50) {
            penaltyPieces.push(Math.random() < 0.5 ? "L" : "J");
            penaltyPieces.push(Math.random() < 0.5 ? "T" : "I");
        }
        if (cancelStreak >= 60) penaltyPieces.push(Math.random() < 0.5 ? "S" : "Z");

        if (penaltyPieces.length > 0) {
            penaltyPieces = penaltyPieces.shuffle();
        }

        shuf = [...shuf, ...penaltyPieces];
...
function callback(gravity=700, special_restart=false, cheese=false) {
	...
	function hardDrop() {
		if (yPOS != yGHO) {
			if (lastAction != 'ROT') lastAction = 'HD';
		}
		yPOS = yGHO;
		held = false;
